...
class EM_EVENT_IVS_TYPE(IntEnum):
    ...
    TRAFFICJUNCTION = (
        0x00000017,
    )  # 交通路口事件(对应 DEV_EVENT_TRAFFICJUNCTION_INFO);traffic junction event(Corresponding to DEV_EVENT_TRAFFICJUNCTION_INFO)
    FACEDETECT = (
        0x0000001A,
    )  # 人脸检测事件(对应 NET_DEV_EVENT_FACEDETECT_INFO); face detection(Corresponding to NET_DEV_EVENT_FACEDETECT_INFO)
...
class EM_LOGIN_SPAC_CAP_TYPE(IntEnum):
    ...
    TCP = 1  # TCP登陆, 默认方式;TCP login, default
...
class SDK_RealPlayType(IntEnum):
    ...
    Realplay_0 = (
        2,
    )  # 实时监视 - 主码流, 等同于Realplay;Real-time monitor-main stream. It is the same as DH_RType_Realplay
...
class EM_PLATE_TYPE(IntEnum):
    ...
    TEMPORARY_LICENSE_FOR_NON_MOTOR_VEHICLES = (
        15,
    )  # 非机动车临时牌照;Temporary license for non motor vehicles
    OFFICIAL_LICENSE_PLATE_FOR_NON_MOTOR_VEHICLE = (
        16,
    )  # 非机动车牌正式牌照;Official license plate of non motor vehicle
...
class EM_USE_PROPERTY_TYPE(IntEnum):
    ...
    SECURITY = (
        6,
    )  # 警用,消防,救护,工程救险;for police,for fire police,for rescue or engineering emergency
    COACH = (7,)  # 教练;for coach
    SCHOOLBUS = (
        8,
    )  # 幼儿校车,小学生校车,其他校车;kindergarten school bus,pupil school bus,other school bus
...
class EM_CLASS_TYPE(IntEnum):
    ...
    GENERATEGRAPHDETECTION = (
        47,
    )  # 生成图规则 "GenerateGraphDetection";GenerateGraphDetection
...
class EM_USEDEV_MODE(IntEnum):
    ...
    TALK_ENCODE_TYPE = (
        2  # 设置语音对讲编码格式(对应DHDEV_TALKDECODE_INFO)； Set encode format for audio talk
    )
    ALARM_LISTEN_MODE = 3  # 设置报警订阅方式； Set alarm subscribe way
    CONFIG_AUTHORITY_MODE = (
        4  # 设置通过权限进行配置管理； Set user right to realize configuration management
    )
    ...
    PLAYBACK_REALTIME_MODE = (
        10  # 设置实时回放功能(0-关闭,1开启)； set real time playback function(0-off, 1-on)
    )
    ...
    AUDIO_RECORD_LENGTH = (
        15  # 设置录音缓存, 对应为一个int； set audio record length, corresponding to a int
    )
...
class EM_QUERY_RECORD_TYPE(IntEnum):
    ...
    CARD_PICTURE = (
        8,
    )  # 按卡号查询图片, HB - U、NVS等使用; query pictures by the card number, used by HB-U,NVS
    ...
    NET_DATA = (
        15,
    )  # 查询网络数据, 金桥网吧等使用; query network data, used by Jinqiao Internet Bar
...
class EM_PERSON_FEATURE_STATE(IntEnum):
    ...
    UNUSEFUL = (
        4,
    )  # 已建模，但算法升级导致数据不可用，需要重新建模;once modeling was successful, but became unusable after upgrading, need to abstract
