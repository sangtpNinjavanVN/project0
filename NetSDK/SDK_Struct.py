...
class NETSDK_INIT_PARAM(Structure):
    ...
    _fields_ = [
        (
            "nThreadNum",
            c_int,
        ),  # 指定NetSDK常规网络处理线程数, 当值为0时, 使用内部默认值; specify netsdk's normal network process thread number, zero means using default value
...
class NET_PARAM(Structure):
    ...
    _fields_ = [
        (
            "nWaittime",
            c_int,
        ),  # 等待超时时间(毫秒为单位),为0默认5000ms;Waiting time(unit is ms), 0:default 5000ms.
        (
            "nConnectTime",
            c_int,
        ),  # 连接超时时间(毫秒为单位),为0默认1500ms;Connection timeout value(Unit is ms), 0:default 1500ms.
        ("nConnectTryNum", c_int),  # 连接尝试次数,为0默认1次;Connection trial times, 0:default 1.
        (
            "nSubConnectSpaceTime",
            c_int,
        ),  # 子连接之间的等待时间(毫秒为单位),为0默认10ms;Sub-connection waiting time(Unit is ms), 0:default 10ms.
        (
            "nGetDevInfoTime",
            c_int,
        ),  # 获取设备信息超时时间,为0默认1000ms;Access to device information timeout, 0:default 1000ms.
        (
            "nConnectBufSize",
            c_int,
        ),  # 每个连接接收数据缓冲大小(字节为单位),为0默认250*1024;Each connected to receive data buffer size(Bytes), 0:default 250*1024
        (
            "nGetConnInfoTime",
            c_int,
        ),  # 获取子连接信息超时时间(毫秒为单位),为0默认1000ms;Access to sub-connect information timeout(Unit is ms), 0:default 1000ms.
        (
            "nSearchRecordTime",
            c_int,
        ),  # 按时间查询录像文件的超时时间(毫秒为单位),为0默认为3000ms;Timeout value of search video (unit ms), default 3000ms
        (
            "nsubDisconnetTime",
            c_int,
        ),  # 检测子链接断线等待时间(毫秒为单位),为0默认为60000ms;dislink disconnect time,0:default 60000ms
        ("byNetType", c_ubyte),  # 网络类型, 0-LAN, 1-WAN;net type, 0-LAN, 1-WAN
        (
            "byPlaybackBufSize",
            c_ubyte,
        ),  # 回放数据接收缓冲大小（M为单位）,为0默认为4M;playback data from the receive buffer size(m),when value = 0,default 4M
        (
            "bDetectDisconnTime",
            c_ubyte,
        ),  # 心跳检测断线时间(单位为秒),为0默认为60s,最小时间为2s;Pulse detect offline time(second) .When it is 0, the default setup is 60s, and the min time is 2s
        (
            "bKeepLifeInterval",
            c_ubyte,
        ),  # 心跳包发送间隔(单位为秒),为0默认为10s,最小间隔为2s;Pulse send out interval(second). When it is 0, the default setup is 10s, the min internal is 2s.
        (
            "nPicBufSize",
            c_int,
        ),  # 实时图片接收缓冲大小（字节为单位）,为0默认为2*1024*1024;actual pictures of the receive buffer size(byte)when value = 0,default 2*1024*1024
...
class NET_DEVICEINFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "byDVRType",
            c_ubyte,
        ),  # DVR类型,见枚举 NET_DEVICE_TYPE DVR type.Please refer to NET_DEVICE_TYPE
        (
            "byChanNum",
            c_ubyte,
        ),  # DVR通道个数,登陆成功时有效,当登陆失败原因为密码错误时,通过此参数通知用户,剩余登陆次数,为0时表示此参数无效; DVR channel amount,When login failed due to password error, notice user via this parameter, remaining login times, is 0 means this parameter is invalid
    ]
...
class LOG_SET_PRINT_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "szLogFilePath",
            c_char * 260,
        ),  # 日志路径(默认"./sdk_log/sdk_log.log");log path(default"./sdk_log/sdk_log.log")
        ("bSetFileSize", c_int),  # 是否重设日志文件大小;reset log size
        (
            "nFileSize",
            c_uint,
        ),  # 每个日志文件的大小(默认大小10240), 单位:KB;each log file size(default size 10240), unit:KB
        ...
        (
            "nPrintStrategy",
            c_uint,
        ),  # 日志输出策略, 0:输出到文件(默认); 1:输出到窗口;log out strategy, 0: output to file(defualt); 1:output to window
        ("byReserved", c_ubyte * 4),  # 字节对齐;Byte alignment
        (
            "cbSDKLogCallBack",
            WINFUNCTYPE(c_int, c_char_p, c_uint, C_LDWORD),
        ),  # 日志回调，需要将sdk日志回调出来时设置，默认为None,对应SDK_Callback的fSDKLogCallBack;log callback, (default None),corresponding to SDK_Callback's fSDKLogCallBack
...
class DEVICE_NET_INFO_EX(Structure):
    ...
    _fields_ = [
        ...
        (
            "szIP",
            c_char * 64,
        ),  # IP IPV4形如"192.168.0.1" IPV6形如"2008::1/64",;IP IPV4 likes "192.168.0.1" ,IPV6 likes "2008::1/64"
        ...
        (
            "byManuFactory",
            c_ubyte,
        ),  # 目标设备的生产厂商,具体参考sdk_enum.py的EM_IPC_TYPE;Manu factory,refer to EM_IPC_TYPE in sdk_enum.py
        ...
        (
            "szUserName",
            c_char * 16,
        ),  # 登陆设备用户名（在修改设备IP时需要填写）;user name for log in device(it need be filled when modify device ip)
        (
            "szPassWord",
            c_char * 16,
        ),  # 登陆设备密码（在修改设备IP时需要填写）;pass word for log in device(it need be filled when modify device ip)
        ...
        (
            "byPwdResetWay",
            c_ubyte,
        ),  # 支持密码重置方式：按位确定密码重置方式，只在设备有初始化账号时有意义;the way supported for reset password:make sense when the device is init
        ...
        (
            "szNewUserName",
            c_char * 64,
        ),  # 登陆设备用户名（在修改设备IP时需要填写）;new user name for login device(it need be filled when modify device ip)
        (
            "byPwdFindVersion",
            c_ubyte,
        ),  # 密码找回的版本号,设备支持密码重置时有效;;password find version, effective when device supports reset password
        ...
        (
            "szDeviceID",
            c_char * 24,
        ),  # 定制字段, 不作为通用协议，不对接通用客户端;Custom item, do not use for general client
        (
            "dwUnLoginFuncMask",
            C_DWORD,
        ),  # Bit0 Wifi列表扫描及WLan设置,Bit1 支持会话外修改过期密码;function mask before login, Bit0 means wifi config
...
class NET_DEVICEINFO_Ex(Structure):
    ...
    _fields_ = [
        ...
        (
            "byLimitLoginTime",
            c_char,
        ),  # 在线超时时间,为0表示不限制登陆,非0表示限制的分钟数;Online Timeout, Not Limited Access to 0, not 0 Minutes Limit Said
        (
            "byLeftLogTimes",
            c_char,
        ),  # 当登陆失败原因为密码错误时,通过此参数通知用户,剩余登陆次数,为0时表示此参数无效; When login failed due to password error, notice user via this parameter, remaining login times, is 0 means this parameter is invalid
        ("bReserved", c_char * 2),  # 保留字节,字节对齐;keep bytes, bytes aligned
        (
            "nLockLeftTime",
            c_int,
        ),  # 当登陆失败,用户解锁剩余时间（秒数）, -1表示设备未设置该参数;when log in failed, the left time for users to unlock (seconds), -1 indicate the device haven't set the parameter
...
class NET_IN_LOGIN_WITH_HIGHLEVEL_SECURITY(Structure):
    ...
    _fields_ = [
        ...
        (
            "emSpecCap",
            c_int,
        ),  # 登录模式,具体信息见sdk_enum.py内的EM_LOGIN_SPAC_CAP_TYPE;Spec login cap，refer to EM_LOGIN_SPAC_CAP_TYPE in sdk_enum.py
        ("byReserved", c_ubyte * 4),  # 保留字节;Reserved
        (
            "pCapParam",
            c_void_p,
        )  # emSpecCap = 0,pCapParam:None;emSpecCap = 0,pCapParam:None
...
class NET_IN_STARTSERACH_DEVICE(Structure):
    ...
    _fields_ = [
        ...
        ),  # 设备信息回调函数;search device call back
        ("pUserData", c_void_p),  # 用户自定义数据;user data
        (
            "emSendType",
            c_int,
        ),  # 下发搜索类型,对应EM_SEND_SEARCH_TYPE;send search type,refer to EM_SEND_SEARCH_TYPE
    ]
...
class NET_OUT_STARTSERACH_DEVICE(Structure):
    ...
    _fields_ = [("dwSize", C_DWORD)]  # 结构体大小，ENGLISH_LANG:Structrue size
...
class DEVICE_IP_SEARCH_INFO_IP(Structure):
    ...
    _fields_ = [("IP", c_char * 64)]  # 具体待搜索的IP信息;the IP for search
...
class NET_IN_INIT_DEVICE_ACCOUNT(Structure):
    ...
    _fields_ = [
        ...
        (
            "byPwdResetWay",
            c_ubyte,
        ),  # 设备支持的密码重置方式：搜索设备接口(StartSearchDevicesEx、SearchDevicesByIPs回调函数)返回字段byPwdResetWay的值
...
class NET_OUT_INIT_DEVICE_ACCOUNT(Structure):
    ...
    _fields_ = [("dwSize", C_DWORD)]  # 结构体大小;Structrue size
...
class NET_RECORDFILE_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "driveno",
            c_uint,
        ),  # 磁盘号(区分网络录像和本地录像的类型,0－127表示本地录像,其中64表示光盘1,128表示网络录像); HDD number
        ("startcluster", c_uint),  # 起始簇号; Initial cluster number
        (
            "nRecordFileType",
            c_ubyte,
        ),  # 录象文件类型  0：普通录象；1：报警录象；2：移动检测；3：卡号录象；4：图片, 5: 智能录像, 19: POS录像, 255:所有录像; Recorded file type  0:general record;1:alarm record ;2:motion detection;3:card number record ;4:image ; 19:Pos record ;255:all
        (
            "bImportantRecID",
            c_ubyte,
        ),  # 0:普通录像 1:重要录像; 0:general record 1:Important record
        (
            "bHint",
            c_ubyte,
        ),  # 文件定位索引(nRecordFileType==4<图片>时,bImportantRecID<<8 +bHint ,组成图片定位索引 ); Document Indexing
        (
            "bRecType",
            c_ubyte,
        ),  # 0-主码流录像 1-辅码1流录像 2-辅码流2 3-辅码流3录像; 0-main stream record 1-sub1 stream record 2-sub2 stream record 3-sub3 stream record
    ]
...
class NET_TIME_EX(Structure):
    ...
    _fields_ = [
        ...
        (
            "dwUTC",
            C_DWORD,
        ),  # utc时间(获取时0表示无效，非0有效,下发无效);utc query: zero means invaild, non-zero means vaild;  set:invalid
...
class SDK_POINT(Structure):
    ...
    _fields_ = [("nx", c_short), ("nx", c_short)]  # x轴;x  # y轴;y
...
class SDK_PIC_INFO(Structure):
    ...
    _fields_ = [
        (
            "dwOffSet",
            C_DWORD,
        ),  # 文件在二进制数据块中的偏移位置, 单位:字节;current picture file's offset in the binary file, byte
        ...
        (
            "bIsDetected",
            c_ubyte,
        ),  # 图片是否算法检测出来的检测过的提交识别服务器时,则不需要再时检测定位抠图,1:检测过的,0:没有检测过;When submit to the server, the algorithm has checked the image or not
        ("bReserved", c_ubyte * 3),  # 预留字节数;reserved data
        (
            "nFilePathLen",
            c_int,
        ),  # 文件路径长度 既pszFilePath 用户申请的大小;File path Len of pszFilePath
        (
            "stuPoint",
            SDK_POINT,
        ),  # 小图左上角在大图的位置，使用绝对坐标系;The upper left corner of the figure is in the big picture. Absolute coordinates are used
    ]
...
class SDK_MSG_OBJECT(Structure):
    ...
    _fields_ = [
        (
            "nObjectID",
            c_int,
        ),  # 物体ID,每个ID表示一个唯一的物体;Object ID,each ID represent a unique object
        ("szObjectType", c_char * 128),  # 物体类型;Object type
        (
            "nConfidence",
            c_int,
        ),  # 置信度(0~255),值越大表示置信度越高;Confidence(0~255),a high value indicate a high confidence
        (
            "nAction",
            c_int,
        ),  # 物体动作:1:Appear 2:Move 3:Stay 4:Remove 5:Disappear 6:Split 7:Merge 8:Rename;Object action:1:Appear 2:Move 3:Stay 4:Remove 5:Disappear 6:Split 7:Merge 8:Rename
        ...
        (
            "Contour",
            SDK_POINT * 16,
        ),  # 较精确的轮廓多边形;a polygon that have a exactitude figure
        (
            "rgbaMainColor",
            C_DWORD,
        ...
        (
            "szText",
            c_char * 128,
        ),  # 物体上相关的带0结束符文本,比如车牌,集装箱号等等;the interrelated text of object,such as number plate,container number
        ...
        (
            "wSubBrand",
            c_ushort,
        ),  # 车辆子品牌 需要通过映射表得到真正的子品牌 映射表详见开发手册;Specifies the sub-brand of vehicle,the real value can be found in a mapping table from the development manual
        ...
        (
            "byTimeType",
            c_ubyte,
        ),  # 时间表示类型,详见EM_TIME_TYPE说明;Time indicates the type of detailed instructions, EM_TIME_TYPE
        (
            "stuCurrentTime",
            NET_TIME_EX,
        ...
        (
            "stuStartTime",
            NET_TIME_EX,
        ),  # 开始时间戳（物体开始出现时);strart time(object appearing for the first time)
        (
            "stuEndTime",
            NET_TIME_EX,
        ),  # 结束时间戳（物体最后出现时）;end time(object appearing for the last time)
        (
            "stuOriginalBoundingBox",
            SDK_RECT,
        ),  # 包围盒(绝对坐标);original bounding box(absolute coordinates)
        ("stuSignBoundingBox", SDK_RECT),  # 车标坐标包围盒;sign bounding box coordinate
        (
            "dwCurrentSequence",
            C_DWORD,
        ),  # 当前帧序号（抓下这个物体时的帧）; The current frame number (frames when grabbing the object)
        (
            "dwBeginSequence",
            C_DWORD,
        ),  # 开始帧序号（物体开始出现时的帧序号）;Start frame number (object appeared When the frame number,
        (
            "dwEndSequence",
            C_DWORD,
        ),  # 结束帧序号（物体消逝时的帧序号）;The end of the frame number (when the object disappearing Frame number)
        (
            "nBeginFileOffset",
            c_int64,
        ),  # 开始时文件偏移, 单位: 字节（物体开始出现时,视频帧在原始视频文件中相对于文件起始处的偏移）;At the beginning of the file offset, Unit: Word Section (when objects began to appear, the video frames in the original video file offset relative to the beginning of the file,
        (
            "nEndFileOffset",
            c_int64,
        ),  # 结束时文件偏移, 单位: 字节（物体消逝时,视频帧在原始视频文件中相对于文件起始处的偏移）;At the end of the file offset, Unit: Word Section (when the object disappeared, video frames in the original video file offset relative to the beginning of the file)
        (
            "byColorSimilar",
            c_ubyte * 8,
        ),  # 物体颜色相似度,取值范围：0-100,数组下标值代表某种颜色,详见EM_COLOR_TYPE;Object color similarity, the range :0-100, represents an array subscript Colors, see EM_COLOR_TYPE,
        (
            "byUpperBodyColorSimilar",
            c_ubyte * 8,
        ),  # 上半身物体颜色相似度(物体类型为人时有效);When upper body color similarity (valid object type man ,
        (
            "byLowerBodyColorSimilar",
            c_ubyte * 8,
        ),  # 下半身物体颜色相似度(物体类型为人时有效);Lower body color similarity when objects (object type human valid ,
        ...
        (
            "wBrandYear",
            c_ushort,
        ),  # 车辆品牌年款 需要通过映射表得到真正的年款 映射表详见开发手册;Specifies the model years of vehicle. the real value can be found in a mapping table from the development manual
    ]
...
class SDK_EVENT_FILE_INFO(Structure):
    ...
    _fields_ = [
        (
            "bCount",
            c_ubyte,
        ),  # 当前文件所在文件组中的文件总数;the file count in the current file's group
        ("bIndex", c_ubyte),  # 当前文件在文件组中的文件编号(编号1开始);the index of the file in the group
        (
            "bFileTag",
            c_ubyte,
        ),  # 文件标签, EM_EVENT_FILETAG;file tag, see the enum struct EM_EVENT_FILETAG
        (
            "bFileType",
            c_ubyte,
        ),  # 文件类型,0-普通 1-合成 2-抠图;file type,0-normal 1-compose 2-cut picture
...
class SDK_RESOLUTION_INFO(Structure):
    ...
    _fields_ = [("snWidth", c_ushort), ("snHight", c_ushort)]  # 宽;width  # 高;hight
...
class EVENT_JUNCTION_CUSTOM_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "nCbirFeatureOffset",
            C_DWORD,
        ),  # 数据偏移，单位字节 （由于结构体保留字节有限的限制,添加在此处， 下同）;Content Based Image Retrieval Feature offset,Unit:Byte
        (
            "nCbirFeatureLength",
            C_DWORD,
        ),  # 数据大小，单位字节;Content Based Image Retrieval Feature length,Unit:Byte
        (
            "dwVehicleHeadDirection",
            C_DWORD,
        ),  # 车头朝向 0:未知 1:左 2:中 3:右;Head direction 0:Unknown 1:left 2:center 3:right
...
class DRIVING_DIRECTION(Structure):
    ...
    _fields_ = [("DrivingDirection", c_char * 256)]  # 行驶方向;Driving direction
...
class SDK_SIG_CARWAY_INFO_EX(Structure):
    ...
    _fields_ = [
        (
            "byRedundance",
            c_ubyte * 8,
        ),  # 由车检器产生抓拍信号冗余信息;The vehicle detector generates the snap signal redundancy info
...
class NET_WHITE_LIST_AUTHORITY_LIST(Structure):
    ...
    _fields_ = [
        (
            "bOpenGate",
            c_int,
        ),  # 是否有开闸权限;true:having open gate authority,false:no having open gate authority
...
class NET_TRAFFICCAR_WHITE_LIST(Structure):
    ...
    _fields_ = [
        (
            "bTrustCar",
            c_int,
        ),  # 车牌是否属于白名单;true: the car is trust car,false:the car is not trust car
...
class NET_TRAFFICCAR_BLACK_LIST(Structure):
    ...
    _fields_ = [
        ...
        (
            "bIsBlackCar",
            c_int,
        ),  # 车牌是否属于黑名单;Whether is the plate on the blacklist or not
...
class DEV_EVENT_TRAFFIC_TRAFFICCAR_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "szPlateType",
            c_char * 32,
        ),  # 号牌类型 "Unknown" 未知; "Normal" 蓝牌黑牌; "Yellow" 黄牌; "DoubleYellow" 双层黄尾牌;Plate type: "Unknown" =Unknown; "Normal"=Blue and black plate. "Yellow"=Yellow plate. "DoubleYellow"=Double-layer yellow plate
        ...
        (
            "szVehicleColor",
            c_char * 32,
        ),  # 车身颜色    "White", "Black", "Red", "Yellow", "Gray", "Blue","Green";vehicle color, "White", "Black", "Red", "Yellow", "Gray", "Blue","Green"
        ("nSpeed", c_int),  # 速度,单位Km/H;speed, Km/H
        (
            "szEvent",
            c_char * 64,
        ),  # 触发的相关事件,参见事件列表Event List,只包含交通相关事件;trigger event type
        ...
        (
            "nVehicleSize",
            c_int,
        ),  # 车辆大小,-1表示未知,否则按位;vehicle size, see VideoAnalyseRule's describe
        ...
        (
            "nSnapshotMode",
            c_int,
        ),  # 抓拍方式,0-未分类,1-全景,2-近景,4-同向抓拍,8-反向抓拍,16-号牌图像;snap mode 0-normal,1-globle,2-near,4-snap on the same side,8-snap on the reverse side,16-plant picture
        (
            "szChannelName",
            c_char * 32,
        ),  # 本地或远程的通道名称,可以是地点信息,来源于通道标题配置ChannelTitle.Name;channel name
        (
            "szMachineName",
            c_char * 256,
        ),  # 本地或远程设备名称,来源于普通配置General.MachineName;Machine name
        (
            "szMachineGroup",
            c_char * 256,
        ),  # 机器分组或叫设备所属单位,默认为空,用户可以将不同的设备编为一组,便于管理,可重复;machine group
        ("szRoadwayNo", c_char * 64),  # 道路编号;road way number
        (
            "szDrivingDirection",
            DRIVING_DIRECTION * 3,
        ),  # 行驶方向 , "DrivingDirection" : ["Approach", "上海", "杭州"];DrivingDirection: for example ["Approach", "Shanghai", "Hangzhou"]
        ...
        (
            "szDeviceAddress",
            c_char_p,
        ),  # 设备地址,OSD叠加到图片上的,来源于配置TrafficSnapshot.DeviceAddress,'\0'结束;device address,OSD superimposed onto the image,from TrafficSnapshot.DeviceAddress,'\0'means end.
        (
            "szVehicleSign",
            c_char * 32,
        ),  # 车辆标识, 例如 "Unknown"-未知, "Audi"-奥迪, "Honda"-本田 ...;Vehicle identification, such as "Unknown" - unknown "Audi" - Audi, "Honda" - Honda ...
        (
            "stuSigInfo",
            SDK_SIG_CARWAY_INFO_EX,
        ),  # 由车检器产生抓拍信号冗余信息;Generated by the vehicle inspection device to capture the signal redundancy
        ("szMachineAddr", c_char_p),  # 设备部署地点;Equipment deployment locations
        (
            "fActualShutter",
            c_float,
        ),  # 当前图片曝光时间,单位为毫秒;Current picture exposure time, in milliseconds
        (
            "byActualGain",
            c_ubyte,
        ),  # 当前图片增益,范围为0~100;Current picture gain, ranging from 0 to 100
        (
            "byDirection",
            c_ubyte,
        ...
        (
            "szDetailedAddress",
            c_char_p,
        ),  # 详细地址, 作为szDeviceAddress的补充;Address, as szDeviceAddress supplement
        ("szDefendCode", c_char * 64),  # 图片防伪码;waterproof
        (
            "nTrafficBlackListID",
            c_int,
        ),  # 关联黑名单数据库记录默认主键ID, 0,无效；> 0,黑名单数据记录;Link black list data recorddefualt main keyID, 0, invalid, > 0, black list data record
        ...
        (
            "szCustomParkNo",
            c_char * 33,
        ),  # 自定义车位号（停车场用）;self defined parking space number, for parking
        ...
        (
            "szCustomRoadwayDirection",
            c_char * 32,
        ),  # 自定义车道方向,byDirection为9时有效;custom road way, valid when byDirection is 9
        (
            "byPhysicalLane",
            c_ubyte,
        ),  # 物理车道号,取值0到5;the physical lane number,value form 0 to 5
        ("byReserved2", c_ubyte * 3),  # 预留字节;Reserved
        (
            "emMovingDirection",
            c_int,
        ),  # 车辆行驶方向,值的意义见EM_TRAFFICCAR_MOVE_DIRECTION;moving direction
        (
            "stuEleTagInfoUTC",
            NET_TIME,
        ),  # 对应电子车牌标签信息中的过车时间(ThroughTime);corresponding to throughTime
        (
            "stuCarWindowBoundingBox",
            NET_RECT,
        ),  # 车窗包围盒，0~8191;The BoundingBox of car window , 0~8191
        ("stuWhiteList", NET_TRAFFICCAR_WHITE_LIST),  # 白名单信息;white list information
        (
            "emCarType",
            c_int,
        ),  # 车辆类型,详见EM_TRAFFICCAR_CAR_TYPE;car type,refer to EM_TRAFFICCAR_CAR_TYPE
        (
            "emLaneType",
            c_int,
        ),  # 车道类型,详见EM_TRAFFICCAR_LANE_TYPE;Lane type,refer to EM_TRAFFICCAR_LANE_TYPE
        (
            "szVehicleBrandYearText",
            c_char * 64,
        ),  # 车系年款翻译后文本内容;Translated year of vehicle
...
class SDK_MSG_OBJECT_EX(Structure):
    ...
    _fields_ = [
        ...
        (
            "nObjectID",
            c_int,
        ),  # 物体ID,每个ID表示一个唯一的物体;object ID, each ID means a exclusive object
        ("szObjectType", c_char * 128),  # 物体类型;object type
        (
            "nConfidence",
            c_int,
        ),  # 置信度(0~255),值越大表示置信度越高;confidence coefficient (0~255),  value the bigger means  confidence coefficient the higher
        (
            "nAction",
            c_int,
        ),  # 物体动作:1:Appear 2:Move 3:Stay 4:Remove 5:Disappear 6:Split 7:Merge 8:Rename;object  motion :1:Appear 2:Move 3:Stay 4:Remove 5:Disappear 6:Split 7:Merge 8:Rename
        ...
        (
            "Contour",
            SDK_POINT * 16,
        ),  # 较精确的轮廓多边形;relatively accurate outline the polygon
        (
            "rgbaMainColor",
            C_DWORD,
        ),  # 表示车牌、车身等物体主要颜色；按字节表示,分别为红、绿、蓝和透明度,例如:RGB值为(0,255,0),透明度为0时, 其值为0x00ff0000.;means plate, vehicle body and etc. object major color, by byte means , are red, green, blue and transparency , such as:RGB value is (0,255,0), transparency is 0, its value is 0x00ff0000.
        (
            "szText",
            c_char * 128,
        ),  # 同SDK_MSG_OBJECT相应字段;same as SDK_MSG_OBJECT corresponding field
        (
            "szObjectSubType",
            c_char * 64,
        ),  # 物体子类别,根据不同的物体类型,可以取以下子类型,同NET_MSG_OBJECT相应字段;object sub type , according to different object  types , may use the following sub type,same as NET_MSG_OBJECT field
        ("byReserved1", c_ubyte * 3),  # 保留字节;Reserved
        (
            "bPicEnble",
            c_bool,
        ),  # 是否有物体对应图片文件信息;object corresponding to picture file info or not
        ...
        (
            "bColor",
            c_bool,
        ),  # 物体颜色(rgbaMainColor)是否可用;object  color (rgbaMainColor) usable or not
        (
            "bLowerBodyColor",
            c_ubyte,
        ),  # 下半身颜色(rgbaLowerBodyColor)是否可用;lower color (rgbaLowerBodyColor) usable or not
        (
            "byTimeType",
            c_ubyte,
        ),  # 时间表示类型,详见EM_TIME_TYPE说明;time means type ,  see EM_TIME_TYPE note
        (
            "stuCurrentTime",
            NET_TIME_EX,
        ),  # 针对视频浓缩,当前时间戳（物体抓拍或识别时,会将此识别智能帧附在一个视频帧或jpeg图片中,此帧所在原始视频中的出现时间）
        ...
        (
            "stuStartTime",
            NET_TIME_EX,
        ),  # 开始时间戳（物体开始出现时）;start time stamp, object start appearance
        (
            "stuEndTime",
            NET_TIME_EX,
        ),  # 结束时间戳（物体最后出现时）;end time stamp, object last aapearance
        ("stuOriginalBoundingBox", SDK_RECT),  # 包围盒(绝对坐标);box(absolute coordinate)
        ("stuSignBoundingBox", SDK_RECT),  # 车标坐标包围盒;LGO coordinate box
        (
            "dwCurrentSequence",
            C_DWORD,
        ),  # 当前帧序号（抓下这个物体时的帧）;current frame no., snapshot this object frame
        (
            "dwBeginSequence",
            C_DWORD,
        ),  # 开始帧序号（物体开始出现时的帧序号）;start frame no., object start appearance frame no.
        (
            "dwEndSequence",
            C_DWORD,
        ),  # 结束帧序号（物体消逝时的帧序号）;end frame no., object disappearance frame no.
        (
            "nBeginFileOffset",
            C_LLONG,
        ...
        (
            "byColorSimilar",
            c_ubyte * 8,
        ),  # 物体颜色相似度,取值范围：0-100,数组下标值代表某种颜色,详见 EM_COLOR_TYPE
        # object  color similarity, take  value range :0-100, group subscript value represents certain color ,  see EM_COLOR_TYPE
        (
            "byUpperBodyColorSimilar",
            c_ubyte * 8,
        ),  # 上半身物体颜色相似度(物体类型为人时有效);upper object  color  similarity (object  type as human is valid )
        (
            "byLowerBodyColorSimilar",
            c_ubyte * 8,
        ),  # 下半身物体颜色相似度(物体类型为人时有效);lower object  color  similarity (object  type as human is valid )
        ...
        (
            "emPersonDirection",
            c_int,
        ),  # 人员入侵方向,详见EM_MSG_OBJ_PERSON_DIRECTION;Staff intrusion direction
        (
            "rgbaLowerBodyColor",
            C_DWORD,
        ),  # 使用方法同rgbaMainColor,物体类型为人时有效;Use direction same as rgbaMainColor,object  type as human is valid
    ]
...
class SDK_EXTRA_PLATE_NUMBER(Structure):
    ...
    _fields_ = [("szNumber", c_char * 32)]  # 额外车牌信息;Extra plate number
...
class EVENT_COMM_SEAT(Structure):
    ...
    _fields_ = [
        ...
        (
            "emSeatType",
            c_int,
        ),  # 座驾类型, 0:未识别; 1:主驾驶; 2:副驾驶,详见EM_COMMON_SEAT_TYPE;seat type,refer to EM_COMMON_SEAT_TYPE
        ("stStatus", EVENT_COMM_STATUS),  # 违规状态;illegal state
        (
            "emSafeBeltStatus",
            c_int,
        ),  # 安全带状态,详见NET_SAFEBELT_STATE;safe belt state,refer to NET_SAFEBELT_STATE
        (
            "emSunShadeStatus",
            c_int,
        ),  # 遮阳板状态,详见NET_SUNSHADE_STATE;sun shade state,refer to NET_SUNSHADE_STATE
...
class NET_RFIDELETAG_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "nCardType",
            c_int,
        ),  # 卡号类型, 0:交通管理机关发行卡, 1:新车出厂预装卡;card type, 0:issued by transport administration offices, 1:new factory preloaded card
        (
            "emCardPrivince",
            c_int,
        ),  # 卡号省份,详见EM_CARD_PROVINCE;card privince,refer to EM_CARD_PROVINCE
        ("szPlateNumber", c_char * 32),  # 车牌号码;plate number
        ...
        (
            "emPlateType",
            c_int,
        ),  # 号牌种类,详见EM_PLATE_TYPE;plate type,refer to EM_PLATE_TYPE
        (
            "szInspectionValidity",
            c_char * 16,
        ),  # 检验有效期，年-月;validity of inspection, year-month
        (
            "nInspectionFlag",
            c_int,
        ),  # 逾期未年检标志, 0:已年检, 1:逾期未年检;the flag of inspetion, 0:already inspection, 1:not inspection
        (
            "nMandatoryRetirement",
            c_int,
        ),  # 强制报废期，从检验有效期开始，距离强制报废期的年数;the years form effective inspection preiod to compulsory discarding preiod
        (
            "emCarColor",
            c_int,
        ),  # 车身颜色，详见EM_CAR_COLOR_TYPE;car color,refer to EM_CAR_COLOR_TYPE
        (
            "nApprovedCapacity",
            c_int,
        ),  # 核定载客量，该值<0时：无效；此值表示核定载客，单位为人;authorized capacity, unit:people, <0:incalid
        (
            "nApprovedTotalQuality",
            c_int,
        ),  # 此值表示总质量，单位为百千克；该值<0时：无效；该值的有效范围为0~0x3FF，0x3FF（1023）表示数据值超过了可存储的最大值;total weight, unit:100kg, range:0~0x3FF,  0x3FF1023:larger than maximum value can be stored, <0:invalid
        ("stuThroughTime", NET_TIME_EX),  # 过车时间;the time when the car is pass
        (
            "emUseProperty",
            c_int,
        ),  # 使用性质,详见EM_USE_PROPERTY_TYPE;use property,refer to EM_USE_PROPERTY_TYPE
        ("szPlateCode", c_char * 8),  # 发牌代号，UTF-8编码;Licensing code, UTF-8 encoding
        (
            "szPlateSN",
            c_char * 16,
        ),  # 号牌号码序号，UTF-8编码;Plate number, serial number, UTF-8 code
        (
            "szTID",
            c_char * 64,
        ),  # 标签(唯一标识), UTF-8编码;Label (Unique identifier), UTF-8 encoding
...
class EVENT_COMM_INFO(Structure):
    ...
    _fields_ = [
        (
            "emNTPStatus",
            c_int,
        ),  # NTP校时状态,详见EM_NTP_STATUS;NTP time sync status,refer to EM_NTP_STATUS
        ("nDriversNum", c_int),  # 驾驶员信息数;driver info number
        ("pstDriversInfo", POINTER(SDK_MSG_OBJECT_EX)),  # 保驾驶员信息数据;driver info data
        (
            "pszFilePath",
            c_char_p,
        ),  # 本地硬盘或者sd卡成功写入路径,为None时,路径不存在;writing path for local disk or sd card, or write to default path if None
        ("pszFTPPath", c_char_p),  # 设备成功写到ftp服务器的路径;ftp path
        (
            "pszVideoPath",
            c_char_p,
        ),  # 当前接入需要获取当前违章的关联视频的FTP上传路径;ftp path for assocated video
        ...
        (
            "fCOPercent",
            c_float,
        ),  # CO所占百分比，单位：% 取值0~100;The percent of CO,unit,% ,range from 0 to 100
        (
            "fCO2Percent",
            c_float,
        ),  # CO2所占百分比，单位：% 取值0~100;The percent of CO2,unit: % ,range from 0 to 100
        (
            "fLightObscuration",
            c_float,
        ),  # 不透光度，单位：% 取值0~100;The obscuration of light,unit,% ,range from 0 to 100
        ...
        (
            "emVehicleTypeInTollStation",
            c_int,
        ),  # 收费站车型分类,详见EM_VEHICLE_TYPE;Vehicle type inToll station,refer to EM_VEHICLE_TYPE
        (
            "emSnapCategory",
            c_int,
        ),  # 抓拍的类型，默认为机动车，详见EM_SNAPCATEGORY;Snap Category;,refer to EM_SNAPCATEGORY
        (
            "nRegionCode",
            c_int,
        ),  # 车牌所属地区代码,(孟加拉海外车牌识别项目),默认-1表示未识别;Location code of license plate,(Bangladesh Project),default -1 indicates unrecognized
        (
            "emVehicleTypeByFunc",
            c_int,
        ),  # 按功能划分的车辆类型，详见EM_VEHICLE_TYPE_BY_FUNC;Vehicle type by function,refer to EM_VEHICLE_TYPE_BY_FUNC
        (
            "emStandardVehicleType",
            c_int,
        ),  # 标准车辆类型，详见EM_STANDARD_VEHICLE_TYPE;Standard vehicle type,refer to EM_STANDARD_VEHICLE_TYPE
        ...
        (
            "emOverseaVehicleCategory",
            c_int,
        ),  # 海外车辆类型中的子类别，详见EM_OVERSEA_VEHICLE_CATEGORY_TYPE;oversea vehicle category,refer to EM_OVERSEA_VEHICLE_CATEGORY_TYPE
...
class NET_FACE_ATTRIBUTE_EX(Structure):
    ...
    _fields_ = [
        ...
        (
            "emComplexion",
            c_int,
        ),  # 肤色,详见EM_COMPLEXION_TYPE;Complexion,refer to EM_COMPLEXION_TYPE
        (
            "emEye",
            c_int,
        ),  # 眼睛状态,详见EM_EYE_STATE_TYPE;Eye state,refer to EM_EYE_STATE_TYPE
        (
            "emMouth",
            c_int,
        ),  # 嘴巴状态,详见EM_MOUTH_STATE_TYPE;Mouth state,refer to EM_MOUTH_STATE_TYPE
        (
            "emMask",
            c_int,
        ),  # 口罩状态,详见EM_MASK_STATE_TYPE;Mask state,refer to EM_MASK_STATE_TYPE
        (
            "emBeard",
            c_int,
        ),  # 胡子状态,详见EM_BEARD_STATE_TYPE;Beard state,refer to EM_BEARD_STATE_TYPE
        (
            "nAttractive",
            c_int,
        ),  # 魅力值, 0未识别，识别时范围1-100,得分高魅力高;Attractive, 0 Not distinguish,Range[1,100]
        ...
        (
            "emStrabismus",
            c_int,
        ),  # 斜视状态,详见EM_STRABISMUS_TYPE;Strabismus,refer to EM_STRABISMUS_TYPE
...
class NET_RIDER_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "emHelmet",
            c_int,
        ),  # 头盔状态,详见EM_NONMOTOR_OBJECT_STATUS;Whether or not wearing a helmet,refer to EM_NONMOTOR_OBJECT_STATUS
        (
            "emCall",
            c_int,
        ),  # 是否在打电话,详见EM_NONMOTOR_OBJECT_STATUS;Whether on the phone,refer to EM_NONMOTOR_OBJECT_STATUS
        (
            "emBag",
            c_int,
        ),  # 是否有背包,详见EM_NONMOTOR_OBJECT_STATUS; Whether or not have bag,refer to EM_NONMOTOR_OBJECT_STATUS
        (
            "emCarrierBag",
            c_int,
        ),  # 有没有手提包,详见EM_NONMOTOR_OBJECT_STATUS;Whether or not have carrierbag,refer to EM_NONMOTOR_OBJECT_STATUS
        (
            "emUmbrella",
            c_int,
        ),  # 是否打伞,详见EM_NONMOTOR_OBJECT_STATUS;Whether an umbrella,refer to EM_NONMOTOR_OBJECT_STATUS
        (
            "emGlasses",
            c_int,
        ),  # 是否有带眼镜,详见EM_NONMOTOR_OBJECT_STATUS; Whether or not wear glasses,refer to EM_NONMOTOR_OBJECT_STATUS
        (
            "emMask",
            c_int,
        ),  # 是否带口罩,详见EM_NONMOTOR_OBJECT_STATUS;Whether to wear a face mask,refer to EM_NONMOTOR_OBJECT_STATUS
        ("emEmotion", c_int),  # 表情,详见EM_EMOTION_TYPE;Emotion,refer to EM_EMOTION_TYPE
        (
            "emUpClothes",
            c_int,
        ),  # 上衣类型,详见EM_CLOTHES_TYPE;UpClothes type,refer to EM_CLOTHES_TYPE
        (
            "emDownClothes",
            c_int,
        ),  # 下衣类型,详见EM_CLOTHES_TYPE;DownClothes type,refer to EM_CLOTHES_TYPE
        (
            "emUpperBodyColor",
            c_int,
        ),  # 上衣颜色,详见EM_OBJECT_COLOR_TYPE;UpClothes color,refer to EM_OBJECT_COLOR_TYPE
        (
            "emLowerBodyColor",
            c_int,
        ),  # 下衣颜色,详见EM_OBJECT_COLOR_TYPE;DownClothes color,refer to EM_OBJECT_COLOR_TYPE
        (
            "bHasFaceImage",
            c_int,
        ),  # 是否有骑车人人脸抠图信息;Whether rider's face image information is contained
        ("stuFaceImage", RIDER_FACE_IMAGE_INFO),  # 骑车人人脸特写描述;Rider face image
        (
            "bHasFaceAttributes",
            c_int,
        ),  # 是否有人脸属性;Whether rider's face Attributes is contained
        ...
        (
            "emHairStyle",
            c_int,
        ),  # 头发样式,详见EM_HAIR_STYLE; Hair style,refer to EM_HAIR_STYLE
...
class NET_NONMOTOR_FEATURE_VECTOR_INFO(Structure):
    ...
    _fields_ = [
        (
            "nOffset",
            c_uint,
        ),  # 非机动车特征值在二进制数据中的偏移, 单位:字节;The offset of non-motor feature data in binary data, unit:bytes
        (
            "nLength",
            c_uint,
        ),  # 非机动车特征值数据长度, 单位:字节;The length of non-motor feature data, unit:bytes
...
class NET_NONMOTOR_PLATE_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "stuBoundingBox",
            NET_RECT,
        ),  # 包围盒， 非机动车矩形框，0~8191相对坐标;BoundingBox Rect, 0~8192
        (
            "stuOriginalBoundingBox",
            NET_RECT,
        ),  # 包围盒， 非机动车矩形框，绝对坐标;BoundingBox Rect, absolute coordinates
...
class EVENT_INTELLI_COMM_INFO(Structure):
    ...
    _fields_ = [
        (
            "emClassType",
            c_int,
        ),  # 智能事件所属大类,详见EM_CLASS_TYPE;class type，refer to EM_CLASS_TYPE
...
class EVENT_PLATE_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "emFrontPlateColor",
            c_int,
        ),  # 车头车牌颜色,详见EM_PLATE_COLOR_TYPE;front plate color,refer to EM_PLATE_COLOR_TYPE
        ("szBackPlateNumber", c_char * 64),  # 车尾车牌号码;back plate number
        (
            "emBackPlateColor",
            c_int,
        ),  # 车尾车牌颜色,详见EM_PLATE_COLOR_TYPE;back plate color,refer to EM_PLATE_COLOR_TYPE
...
class VA_OBJECT_NONMOTOR(Structure):
    ...
    _fields_ = [
        ...
        (
            "stuRiderList",
            NET_RIDER_INFO * 16,
        ),  # 骑车人特征,个数和nNumOfCycling关联;The information of rider
        ...
        (
            "emNonMotorFeatureVersion",
            c_int,
        ),  #  非机动车特征值版本号;Non-motor feature data version
        (
            "stuNomotorPlateInfo",
            NET_NONMOTOR_PLATE_INFO,
        ),  #  非机动车牌信息;Plate info of nomotor
        (
            "stuObjCenter",
            SDK_POINT,
        ),  # 物体型心(不是包围盒中心), 0-8191相对坐标, 相对于大图; Center of object(not center of bounding box), 0-8191 relative coordinates, relative to large graph
...
class DEV_EVENT_TRAFFICJUNCTION_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "byMainSeatBelt",
            c_ubyte,
        ),  # 主驾驶座,系安全带状态,1-系安全带,2-未系安全带;main driver, seat, safety belt , 1-fastened, 2-unfastened
        (
            "bySlaveSeatBelt",
            c_ubyte,
        ),  # 副驾驶座,系安全带状态,1-系安全带,2-未系安全带;co-drvier, seat, safety belt, 1-fastened, 2-unfastened
        (
            "byVehicleDirection",
            c_ubyte,
        ),  # 当前被抓,拍到的车辆是车头还是车尾,具体请见 EM_VEHICLE_DIRECTION;Current snapshot is head or rear, see  EM_VEHICLE_DIRECTION
        (
            "byOpenStrobeState",
            c_ubyte,
        ),  # 开闸状态,具体请见EM_OPEN_STROBE_STATE;Open status, see EM_OPEN_STROBE_STATE
        ...
        (
            "dwBreakingRule",
            C_DWORD,
        ),  # 违反规则掩码,第一位:闯红灯;BreakingRule's mask,first byte: crash red light;
        ...
        (
            "nSequence",
            c_int,
        ),  # 表示抓拍序号,如3,2,1,1表示抓拍结束,0表示异常结束;snap index,such as 3,2,1,1 means the last one,0 means there has some exception and snap stop
        ("nSpeed", c_int),  # 车辆实际速度Km/h;car's speed (km/h)
        (
            "bEventAction",
            c_ubyte,
        ),  # 事件动作,0表示脉冲事件,1表示持续性事件开始,2表示持续性事件结束;Event action,0 means pulse event,1 means continuous event's begin,2means continuous event's end;
        (
            "byDirection",
            c_ubyte,
        ),  # 路口方向,1-表示正向,2-表示反向;Intersection direction 1 - denotes the forward 2 - indicates the opposite
        (
            "byLightState",
            c_ubyte,
        ),  # LightState表示红绿灯状态:0 未知,1 绿灯,2 红灯,3 黄灯;LightState means red light status:0 unknown,1 green,2 red,3 yellow
        ("byReserved", c_ubyte),  # 保留字节;reserved
        (
            "byImageIndex",
            c_ubyte,
        ),  # 图片的序号, 同一时间内(精确到秒)可能有多张图片, 从0开始;Serial number of the picture, in the same time (accurate to seconds) may have multiple images, starting from 0
        ...
        (
            "szRecordFile",
            c_char * 128,
        ),  # 报警对应的原始录像文件信息;Alarm corresponding original video file information
        ("stuCustomInfo", EVENT_JUNCTION_CUSTOM_INFO),  # 报警对应的原始录像文件信息;custom info
        (
            "byPlateTextSource",
            c_ubyte,
        ),  # 车牌识别来源, 0:本地算法识别,1:后端服务器算法识别;the source of plate text, 0:Local,1:Server
        ...
        (
            "byNoneMotorInfo",
            c_ubyte,
        ),  # 0-无非机动车人员信息信息,1-有非机动车人员信息信息;specified the person info of none motor
        # 此字段为1时下面11个字段生效;1 means 11 fields followed is valid
        ("byBag", c_ubyte),  # 是否背包, 0-未知 1-不背包   2-背包;0-unknown 1-no bag   2-bag
        (
            "byUmbrella",
            c_ubyte,
        ),  # 是否打伞, 0-未知 1-不打伞   2-打伞;0-unknown 1-no umbrella   2-Umbrella
        (
            "byCarrierBag",
            c_ubyte,
        ),  # 手提包状态,0-未知 1-没有 2-有;0-unknown 1-no carrierBag 2-carrierBag
        ...
        (
            "byUpClothes",
            c_ubyte,
        ),  # 上身衣服类型 0:未知 1:长袖 2:短袖 3:长裤 4:短裤 5:裙子 6:背心 7:超短裤 8:超短裙;upper clothes 0:unknown 1:long sleeve 2:short sleeve 3:trousers 4:breeches 5:skirt 6:vest 7:minipants 8:miniskirt
        (
            "byDownClothes",
            c_ubyte,
        ),  # 下身衣服类型 0:未知 1:长袖 2:短袖 3:长裤 4:短裤 5:裙子 6:背心 7:超短裤 8:超短裙;lower clothes 0:unknown 1:long sleeve 2:short sleeve 3:trousers 4:breeches 5:skirt 6:vest 7:minipants 8:miniskirt
        ("stuExtensionInfo", NET_EXTENSION_INFO),  # 扩展信息;Extension info
        ("bReserved", c_ubyte * 22),  # 保留字节,留待扩展;Reserved bytes, leave extended
        (
            "nTriggerType",
            c_int,
        ),  # TriggerType:触发类型,0车检器,1雷达,2视频,3RSU;Trigger Type:0 vehicle inspection device, 1 radar, 2 video, 3 RSU
        (
            "stTrafficCar",
            DEV_EVENT_TRAFFIC_TRAFFICCAR_INFO,
        ),  # 交通车辆信息;Traffic vehicle info
...
class EVENT_INFO(Structure):
    ...
    _fields_ = [
        (
            "nEvent",
            c_int,
        ),  # 事件类型,参见智能事件类型，如 EVENT_IVS_ALL;Event type, see intelligent analysis event type,like EVENT_IVS_ALL
        (
            "arrayObejctType",
            c_int * 16,
        ),  # 支持的物体类型，当前支持 EM_OBJECT_TYPE.HUMAN, EM_OBJECT_TYPE.VECHILE, EM_OBJECT_TYPE.NOMOTOR, EM_OBJECT_TYPE.ALL,参考EM_OBJECT_TYPE;object type, currently support EM_OBJECT_TYPE_HUMAN, EM_OBJECT_TYPE_VECHILE, EM_OBJECT_TYPE_NOMOTOR, EM_OBJECT_TYPE_ALL
...
class NET_IN_PLAY_BACK_BY_TIME_INFO(Structure):
    ...
    _fields_ = [
        ...
        ),  # 进度回调，对应SDK_Callback的fDownLoadPosCallBack; Download pos callback，corresponding to SDK_Callback's fDownLoadPosCallBack
        ...
        ),  # 数据回调，对应SDK_Callback的fDataCallBack; Download data callback，corresponding to SDK_Callback's fDataCallBack
        ...
        (
            "pstuEventInfo",
            POINTER(EVENT_INFO),
        ),  # 事件信息（定制），用户分配内存，不用时赋值为NULL; Event info(customized), user allocate memory
        (
            "nEventInfoCount",
            c_uint,
        ),  # pstuEventInfo 个数，最大为 16; pstuEventInfo's count, max num is 16
...
class SNAP_PARAMS(Structure):
    ...
    _fields_ = [
        ...
        (
            "mode",
            c_uint,
        ),  # 抓图模式；-1:表示停止抓图, 0：表示请求一帧, 1：表示定时发送请求, 2：表示连续请求；Snapshot mode;0:request one frame,1:send out requestion regularly,2: Request consecutively
        (
            "InterSnap",
            c_uint,
        ),  # 时间单位秒；若mode=1表示定时发送请求时,只有部分特殊设备(如：车载设备)支持通过该字段实现定时抓图时间间隔的配置
        # Time unit is second.If mode=1, it means send out requestion regularly. The time is valid.
        (
            "CmdSerial",
            c_uint,
        ),  # 请求序列号，有效值范围 0~65535，超过范围会被截断为 unsigned short；Request serial number，valid value:0~65535
...
class ALARM_MOTIONDETECT_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "nEventAction",
            c_int,
        ),  # 事件动作,0表示脉冲事件,1表示持续性事件开始,2表示持续性事件结束;event action, 0 means pulse event, 1 means continuous event begin, 2 means continuous event end;
        ("nRegionNum", c_uint),  # 动检区域个数;count of region
        (
            "stuRegion",
            NET_MOTIONDETECT_REGION_INFO * 32,
        ),  # 动检区域信息;region info of motion detection
        (
            "bSmartMotionEnable",
            c_int,
        ),  # 智能动检是否使能;smart motion detection is enable or not
        ("nDetectTypeNum", c_uint),  # 动检触发类型个数;count of triggeing motion detection type
        (
            "emDetectType",
            c_int * 32,
        ),  # 动检触发类型, 当nRegionNum大于0时，和stuRegion数组一一对应,参考枚举EM_MOTION_DETECT_TYPE;triggeing motion detection type, when nRegionNum>0，one-to-one correspondence with stuRegion if nRegionNum is biger than 0，refer to EM_MOTION_DETECT_TYPE
...
class NET_FEATURE_VECTOR(Structure):
    ...
    _fields_ = [
        (
            "dwOffset",
            C_DWORD,
        ),  # 人脸小图特征值在二进制数据块中的偏移;Face feature data offset in data block(Unit:BYTE)
...
class NET_HUMAN_TEMPERATURE_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "emTemperatureUnit",
            c_int,
        ),  # 温度单位，参考EM_HUMAN_TEMPERATURE_UNIT;Temperature unit
...
class DEV_EVENT_FACEDETECT_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "dwSnapFlagMask",
            C_DWORD,
        ),  # 抓图标志(按位),具体见NET_RESERVED_COMMON;flag(by bit),see NET_RESERVED_COMMON
        (
            "szSnapDevAddress",
            c_char * 260,
        ),  # 抓拍当前人脸的设备地址,如：滨康路37号;snapshot current face device address
        ...
        (
            "nFeatureValidNum",
            c_uint,
        ),  # 人脸特征数组有效个数,与 emFeature 结合使用;invalid number in array emFeature
        (
            "emFeature",
            c_uint * 32,
        ),  # 人脸特征数组,与 nFeatureValidNum 结合使用，参考EM_DEV_EVENT_FACEDETECT_FEATURE_TYPE;human face features
        ("nFacesNum", c_int),  # 指示stuFaces有效数量;number of stuFaces
        (
            "stuFaces",
            NET_FACE_INFO * 10,
        ),  # 多张人脸时使用,此时没有Object;when nFacesNum > 0, stuObject invalid
        ...
        (
            "nAttractive",
            c_int,
        ),  # 魅力值, -1表示无效, 0未识别，识别时范围1-100,得分高魅力高;Attractive value, -1: invalid, 0:no disringuish，range: 1-100, the higher value, the higher charm
        (
            "szUID",
            c_char * 32,
        ),  # 抓拍人员写入数据库的唯一标识符;The unique identifier of the snap person to write to the database
        ("emNation", c_int),  # 民族，参考EM_NATION_TYPE;nation
        ("stuFeatureVector", NET_FEATURE_VECTOR),  # 特征值信息;Feature data information
        (
            "szFeatureVersion",
            c_char * 32,
        ),  # 特征值算法版本;The version of the feature data algorithm
        (
            "emFaceDetectStatus",
            c_int,
        ),  # 人脸在摄像机画面中的状态，参考EM_FACE_DETECT_STATUS;The status of person in camera picture
        (
            "stuFaceCaptureAngle",
            NET_EULER_ANGLE,
        ),  # 人脸在抓拍图片中的角度信息, nPitch:抬头低头的俯仰角, nYaw左右转头的偏航角, nRoll头在平面内左偏右偏的翻滚角;euler angle of face in the capture picture, nPitch:pitch of the head, nYaw: yaw of the head, nRoll:roll of the head
        ...
        (
            "nFaceAlignScore",
            c_int,
        ),  # 人脸对齐得分分数,范围 0~10000,-1为无效值;The score of face picture align.The range is 0~10000,-1 is invalid
        (
            "nFaceClarity",
            c_int,
        ),  # 人脸清晰度分数,范围 0~10000,-1为无效值;The score of face picture clarity.The range is 0~10000,-1 is invalid
        (
            "bHumanTemperature",
            c_int,
        ),  # 人体温信息是否有效;Whether the information of human body temperature is valid
        (
            "stuHumanTemperature",
            NET_HUMAN_TEMPERATURE_INFO,
        ),  # 人体温信息, bHumanTemperature为TURE时有效;Information of human body temperature, It is valid whne bHumanTemperature is TURE
...
class FACERECOGNITION_PERSON_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "bImportantRank",
            c_ubyte,
        ),  # 人员重要等级,1~10,数值越高越重要,作为查询条件时,此参数填0,则表示此参数无效;importance level,1~10,the higher value the higher level
        ...
        (
            "byType",
            c_ubyte,
        ),  # 人员类型,详见 EM_PERSON_TYPE;Personnel types, see EM_PERSON_TYPE
        (
            "byIDType",
            c_ubyte,
        ),  # 证件类型,详见 EM_CERTIFICATE_TYPE;Document types, see EM_CERTIFICATE_TYPE
        (
            "byGlasses",
            c_ubyte,
        ),  # 是否戴眼镜，0-未知 1-不戴 2-戴;Whether wear glasses or not,0-unknown,1-not wear glasses,2-wear glasses
        ...
        (
            "szPersonNameEx",
            c_char * 64,
        ),  # 姓名,因存在姓名过长,16字节无法存放问题,故增加此参数,;Name, the name is too long due to the presence of 16 bytes can not be Storage problems, the increase in this parameter
        ("szUID", c_char * 32),  # 人员唯一标识符,首次由服务端生成,区别于ID字段,修改,删除操作时必填;person unique ID
        ("szCountry", c_char * 3),  # 国籍,符合ISO3166规范;country
        (
            "byIsCustomType",
            c_ubyte,
        ),  # 人员类型是否为自定义: 0 使用Type规定的类型 1 自定义,使用szPersonName字段;using person type: 0 using byType, 1 using szPersonName
        (
            "pszComment",
            c_char_p,
        ),  # 备注信息, 用户自己申请内存的情况时;comment info, when the memory is alloced by user,
        ...
        (
            "pszGroupID",
            c_char_p,
        ),  # 人员所属组ID, 用户自己申请内存的情况时;group ID, when the memory is alloced by user,
        ...
        (
            "pszGroupName",
            c_char_p,
        ),  # 人员所属组名, 用户自己申请内存的情况时;group name, when the memory is alloced by user,
        ...
        (
            "pszFeatureValue",
            c_char_p,
        ),  # 人脸特征, 用户自己申请内存的情况时;the face feature , when the memory is alloced by user,
...
class FACERECOGNITION_PERSON_INFOEX(Structure):
    ...
    _fields_ = [
        ...
        (
            "byIsCustomType",
            c_ubyte,
        ),  # 人员类型是否为自定义: 0 使用Type规定的类型 1 自定义,使用szCustomType字段;using person type: 0 using byType, 1 using szCustomType
        ...
        (
            "nAttractive",
            c_int,
        ),  # 魅力值, -1表示无效, 0未识别，识别时范围1-100,得分高魅力高;attractive, -1:invalid, 0:unknown，1-100
        ...
        (
            "nEmotionValidNum",
            c_int,
        ),  # 人脸特征数组有效个数,与 emFeature 结合使用, 如果为0则表示查询所有表情;invalid number in array emEmotion, 0 means all emotion
        (
            "emEmotions",
            c_int * 32,
        ),  # 人脸特征数组,与 byFeatureValidNum 结合使用  设置查询条件的时候使用，参考EM_DEV_EVENT_FACEDETECT_FEATURE_TYPE;human emotion  set the query condition
        (
            "nCustomPersonInfoNum",
            c_int,
        ),  # 注册人员信息扩展个数;extension number of registered personnel information
        (
            "szCustomPersonInfo",
            CUSTOM_PERSON_INFO * 4,
        ),  # 注册人员信息扩展;extension of registered personnel information
        (
            "emRegisterDbType",
            c_int,
        ),  # 注册库类型，参考EM_REGISTER_DB_TYPE;type of register face DB
        ("stuEffectiveTime", NET_TIME),  # 有效期时间;effective time
        (
            "emFeatureErrCode",
            c_int,
        ),  # 建模失败原因，参考EM_PERSON_FEATURE_ERRCODE;error code of person feature
...
class CANDIDATE_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "byRange",
            c_ubyte,
        ),  # 人员所属数据库范围,详见EM_FACE_DB_TYPE; Range officer's database, see EM_FACE_DB_TYPE
        ("byReserved1", c_ubyte * 2),  # 预留字段;Reserved
        (
            "stTime",
            NET_TIME,
        ),  # 当byRange为历史数据库时有效,表示查询人员出现的时间;When byRange historical database effectively, which means that the query time staff appeared
        (
            "szAddress",
            c_ubyte * 260,
        ),  # 当byRange为历史数据库时有效,表示查询人员出现的地点;When byRange historical database effectively, which means that people place a query appears
        (
            "bIsHit",
            c_int,
        ),  # 是否有识别结果,指这个检测出的人脸在库中有没有比对结果;Is hit, means the result face has compare result in database
...
class NET_HISTORY_HUMAN_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "emHasCarrierBag",
            c_int,
        ),  # 是否带手提包，参考EM_HAS_CARRIER_BAG;Has carrier bag or not
        (
            "emHasShoulderBag",
            c_int,
        ),  # 是否有肩包，参考EM_HAS_SHOULDER_BAG;Has shoulder bag or not
        (
            "emMessengerBag",
            c_int,
        ),  # 是否有斜跨包，参考EM_HAS_MESSENGER_BAG;Has messenger bag or not
...
class NET_FACE_DATA(Structure):
    ...
    _fields_ = [
        ...
        (
            "stuFaceCaptureAngle",
            NET_EULER_ANGLE,
        ),  # 人脸在抓拍图片中的角度信息,角度值取值范围[-90,90], 三个角度值都为999表示此角度信息无效; euler angle of face in the capture picture,range of the angle value is [-90,90], stuFaceCaptureAngle is invalid if the three angles are 999.
...
class NET_PASSERBY_INFO(Structure):
    ...
    _fields_ = [
        (
            "szPasserbyUID",
            c_char * 32,
        ),  # 路人唯一标识符;The unique identifier of the passerby to write to the database
...
class DEV_EVENT_FACERECOGNITION_INFO(Structure):
    ...
    _fields_ = [
        ...
        (
            "szSnapDevAddress",
            c_char * 260,
        ),  # 抓拍当前人脸的设备地址,如：滨康路37号;Snapshot current face aadevice address
        ("nOccurrenceCount", c_uint),  # 事件触发累计次数;event trigger accumilated times
        (
            "stuIntelliCommInfo",
            EVENT_INTELLI_COMM_INFO,
        ),  # 智能事件公共信息;intelligent things info
        ...
        (
            "szSourceID",
            c_char * 32,
        ),  # 事件关联ID,同一个物体或图片生成多个事件时SourceID相同;Correlate event ID, events arising from same object or picture could have same correlate event ID
        ...
        (
            "nRetCandidatesExNum",
            c_int,
        ),  # 当前人脸匹配到的候选对象数量实际返回值;the actual return number of stuCandidatesEx
        (
            "stuCandidatesEx",
            CANDIDATE_INFOEX * 50,
        ),  # 当前人脸匹配到的候选对象信息扩展;the expansion of candidate information
